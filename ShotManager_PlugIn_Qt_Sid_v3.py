...
uiPath = "C:/QtSDK/QtCreato/bin/SidTest2/mainwindow.ui"
...
class shotManager_SID:
	def __init__(self):
		...
		cmds.optionMenu("deptList", e=1 , changeCommand = lambda event:runPutDeptComponentsAndInvisible())
		cmds.button("btnNewFile", e=1, command = lambda event:runCreateAsset())
		cmds.optionMenu("charBgList", e=1, changeCommand = lambda event:runchangeCharBgComboAndInvisibleFromCharBg())
		cmds.optionMenu("sceneList", e=1, changeCommand = lambda event:runchangeCharBgComboAndInvisible())
		cmds.optionMenu("versionList", e=1, changeCommand = lambda event:runChangeVersionList())
		cmds.button("btnLoadVersion", e=1, command = lambda event:runLoadAndVisible())
		cmds.button("btnLoadLatest", e=1, command = lambda event:runLoadLatestAndVisible())
		cmds.button("btnSaveNetwork", e=1, command = lambda event:saveToNWFunction())
		cmds.button("btnSaveWs", e=1, command = lambda event:saveToWorkspaceFunction())
